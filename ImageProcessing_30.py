...
def process_contours(contours, mask, original_image):
    ...
    for contour in contours:
        ...
        if 100 < area < 1000:  # 调整面积阈值
            x, y, w, h = cv2.boundingRect(contour)
            aspect_ratio = w / h
            if 0.8 < aspect_ratio < 1.2:  # 长宽比约束
                cv2.rectangle(enhanced_mask, (x, y), (x + w, y + h), (0, 255, 0), 2)
        elif area > 1000:  # 针对大目标进行分割
            x, y, w, h = cv2.boundingRect(contour)
...
def texture_analysis(image, mask):
    ...
    laplacian_mask = cv2.inRange(laplacian, 50, 255)  # 提高边缘响应阈值
